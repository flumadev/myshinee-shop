...
const Home: NextPage = () => {
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    const emptyItems = Object.values(formValues).filter(
      (item) => item.length > 0
    );
  ...
  const pay = async () => {
    ...
    if (paymentMethod === 'pix') {
      try {
        const response = await axios.post(
          `${process.env.BASE_URL}/products/pay/pix/${id}`,
          { ...clientData, cupomCode }
        );
    ...
    if (paymentMethod === 'credit') {
      try {
        const response = await axios.post(
          `${process.env.BASE_URL}/products/pay/card/credit/${id}`,
          { ...clientData, cardData, cupomCode }
        );
        if (response.data.status === 'failed') {
          toast({
            title: 'Pagamento não autorizado',
            description:
              'Verifique os dados e tente novamente ou entre em contato com seu banco.',
            status: 'error',
            duration: 9000,
            isClosable: true,
          });
  ...
  const applyCupom = async () => {
    ...
    const cupomResponseData = await axios.get(
      `${process.env.BASE_URL}/cupons/${cupomCode}`
    );
        ...
        >
              ...
              <HStack>
                ...
                <FormControl id="phone" isRequired w="250px">
                  <Input
                    ...
                    id="phone"
                    name="phone"
                    onChange={(e) => setPhone(celularMask(e.target.value))}
                    value={phone}
                  />
                </FormControl>
              </HStack>
          ...
          >
            ...
            >
              <HStack gap={'6px'}>
                <PaymentCard
                  clicked={paymentMethod === 'credit' && canPay}
                  icon={credit}
                  text="Crédito"
                  onClick={() => {
                ...
                />
                <PaymentCard
                  clicked={paymentMethod === 'pix' && canPay}
                  icon={pix}
                  text="Pix"
                  onClick={() => {
                    ...
                    <FormControl isRequired>
                      <Input
                        ...
                        onChange={(e) =>
                          setCardData({
                            ...cardData,
                            cardNumber: cardNumberMask(e.target.value),
                          })
                        }
                  ...
                  <HStack>
                    <FormControl isRequired>
                      <Input
                        ...
                        id="expire"
                        name="expire"
                        value={cardData.cardExpire}
                        onChange={(e) =>
                          setCardData({
                            ...cardData,
                            cardExpire: cardExpireMask(e.target.value),
                          })
                        }
                      />
                    </FormControl>
                          ...
                          {new Array(10).fill(1).map((item, i) => {
                            return (
                              <option key={i} value={i + 1}>
                                {i + 1}x{' '}
                                {formatCurrency(
                                  parseFloat(total.replace('R$', '')) / (i + 1)
                                )}
                              </option>
                            );
                          })}
          ...
          <Container borderRadius={'md'} bgColor="gray.100" padding={'6'}>
            ...
            {canPay && paymentMethod === 'pix' && (
              <Container padding={0} fontSize={{ base: '14px', sm: '16px' }}>
                ...
                <HStack my={'3'} justifyContent={'space-between'}>
                  <Text>{`${clientData.street}, N°${clientData.number} ${
                    clientData.complement ? ', ' + clientData.complement : ''
                  }, ${clientData.district} - ${clientData.city}, ${
                    clientData.state
                  }`}</Text>
                </HStack>
              </Container>
